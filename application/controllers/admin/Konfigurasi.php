 ...
 // New logo
  public function footer() {
    $konfigurasi = $this->konfigurasi_model->listing();
      ...
      $config['max_size']     = '12000'; // KB  
      $this->load->library('upload', $config);
      if(! $this->upload->do_upload('g_footer1')) {
        
    $data = array(  'title' => 'New footer',
            'konfigurasi'  => $konfigurasi,
            'error' => $this->upload->display_errors(),
            'isi' => 'admin/konfigurasi/g_footer1');
    $this->load->view('admin/konfigurasi/footer',$data);
    }else{
        $upload_data        = array('uploads' =>$this->upload->data());
        // Image Editor
        $config['image_library']  = 'gd2';
        ...
        $this->image_lib->resize();
        // Hapus gambar lama
        if($konfigurasi->g_footer1 != ""){
        unlink('./assets/upload/image/'.$konfigurasi->g_footer1);
        unlink('./assets/upload/image/thumbs/'.$konfigurasi->g_footer1);
      }
        // End hapus gambar lama
        // Masuk ke database
        $i = $this->input;
        $data = array(  'id_konfigurasi'=> $i->post('id_konfigurasi'),
                'g_footer1'      => $upload_data['uploads']['file_name'],
                'id_user'     => $this->session->userdata('id'));
        $this->konfigurasi_model->edit($data);
        $this->session->set_flashdata('sukses','g_footer1 changed');
    ...
    // Default page
     $config['upload_path']    = './assets/upload/image/';
      ...
      $config['max_size']     = '12000'; // KB  
      $this->load->library('upload', $config);
      if(! $this->upload->do_upload('g_footer2')) {
        
    $data = array(  'title' => 'New footer',
            'konfigurasi'  => $konfigurasi,
            'error' => $this->upload->display_errors(),
            'isi' => 'admin/konfigurasi/g_footer2');
    }else{
        $upload_data        = array('uploads' =>$this->upload->data());
        // Image Editor
        $config['image_library']  = 'gd2';
        ...
        $this->image_lib->resize();
        // Hapus gambar lama
        if($konfigurasi->g_footer2 != ""){
        unlink('./assets/upload/image/'.$konfigurasi->g_footer2);
        unlink('./assets/upload/image/thumbs/'.$konfigurasi->g_footer2);
      }
        // End hapus gambar lama
        // Masuk ke database
        $i = $this->input;
        $data = array(  'id_konfigurasi'=> $i->post('id_konfigurasi'),
                'g_footer2'      => $upload_data['uploads']['file_name'],
                'id_user'     => $this->session->userdata('id'));
        $this->konfigurasi_model->edit($data);
        $this->session->set_flashdata('sukses','g_footer2 changed');
        redirect(base_url('admin/konfigurasi/footer'));
    }
    // Default page
    $konfigurasi = array(  
